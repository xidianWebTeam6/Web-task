...
<%if len(app)=0 then
sql="select * from jaco order by time DESC,tend DESC"
else
	if wd="" or vs="不限活动开始时间" or vs="" or zz="" or xb="" then
	sql="select * from jaco order by time DESC,tend DESC"
	else
	msqls="select * from jaco where 1+1"
	'以下调用上面的函数即可,可以很多个调用（理论上是任意）
	msqls=msql("day",wd,msqls)
	msqls=msql("myid",zz,msqls)
			 if xb=1 then
			 msqls=msqls & " and datediff('s',tend,now())<0 "
			 elseif xb=2 then
			 msqls=msqls & " and datediff('s',tend,now())>0 "
			 end if
	msqls=msqls & " order by tend DESC,time DESC"
	sql=msqls
	end if
end if
...
共<%=maxpage%>页(<%
		 ...
		 for i=1 to maxpage
			if i=requestpage then
				...
				if len(app)=0 then
				response.write "<a href='"&weburl&"?m=index&a=index&p="&i&"'>["&i&"]</a> "
				else
				response.write "<a href='"&weburl&"?m=index&a=index&org="&zz&"&vsclass="&vs&"&week="&wd&"&oc="&xb&"&p="&i&"'>["&i&"]</a>"
				end if
			end if 
		 next
