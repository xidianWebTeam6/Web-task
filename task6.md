...
内容设计
========
对于一个网站来说，内容就是生命。网站内容的质量直接决定着网站的来访量以及由此带来的经济效益和社会影响力。可以说，好的内容既可以使得网站具备光鲜亮丽的外表，又能赐予网站深刻厚重的灵魂，所以“内容为王”，要想孕育一个好的网站，内容绝对是第一要义。对于我们的招募系统来说，如何确保内容表达和传递等环节上的质量，同样需要设计者开发者维护者管理者仔细思考。

一．	内容的标题

1．	真实、准确、新颖：作为一个招募志愿者的网站，其主体内容一定是各种各样的招募信息。为了使得这些信息能最有效地被志愿者获取，就要求由志愿者招募团体发布的新闻必须是真实而准确的，这两个标准会直接影响招募团体的信用评级。在确保这两个标准的前提下，如能使得标题更加富有创意，自然会招徕到更多的志愿者的点击从而使他们参与到相应的志愿活动中去。

2．	字数控制：字数控制是标题的要求之一，要做到搜索引擎能够记录，并保证收录显示出来的快照能够完整地显示出某个招募信息的标题即可，这样做的好处就是方便站外的志愿者能够通过搜索引擎找到相关的招募信息，从而加入到志愿活动中来。一个简洁高热度的标题能够帮助某个招募团体扩大自己的宣传范围。

3．	符号控制：内容页面一般不带符号，如果需要，可灵活使用一些感染类、疑问类的符号，达到吸引浏览者的目的就行。

二．	内容的主题

我们的招募网站内容主要由个人主页和消息发布面板组成。个人主页其实就是一个博客，上面主要是志愿者们参加志愿活动后的一个反馈，包含图片和文字等信息；消息发布面板就是各招募团体发布招募信息的平台。我们针对这主要的内容载体提出以下的设计思路，总体的设计目标就是“让更多的人看到”：

1．文字说明：文字是内容的第一载体，有些信息必须由文字来表达才能变得准确。对于招募活动信息来说，要准确地用文字交代好本次志愿活动的时间、地点和目标，如何更好地用文字来招募到更多的志愿者，则是新闻发布者要仔细考虑的事情。对于志愿者来说，一个好的反馈与记录我想是一份宝贵的财富，让别人在分享的同时获得精神上的震撼也从而让更多的人加入到志愿者的行列本来就是我们构建这个系统的初衷。

2．图片说明：图片是辅助介绍内容的一个非常好的方式，清晰的图片，周到的布置，不仅让读者增加了市局效果，更能相应的辅助我们所写的内容，从而增加某个志愿的活动的亲历感，拉近读者和志愿者之间的距离。

3．视频效果：视频是一个新生的文章主体，我们的网站能够保证足够的带宽、空间和流量。我相信相应的视频会起到声色并茂的效果，从而增强我们文章内容的说服力，是的文章的力度提高一个层次。

4．适当评论：评论是内容页不断互动不断更新的最大的来源，这个不仅能够达到内容主体的讨论，更能有益补充我们内容的质量。但是这里要说明的是注意自己的评论内容不要出现敏感词语和其他违反互联网法规的词语，一定要做好保护措施。

5．推荐插件：要让更多的人了解到志愿活动的发布和具体的信息，文章的转载功能必不可少，所以我们需要一些微博或博客的插件。

三．	主体相关链接

1． 完全相关链接：完全相关链接，一定要记得链接到和我们网站相关的页面（比如其他的志愿者招募网站），这样会更能增加我们相关页面的权重，但是这里注意的是不管是完全相关还是泛相关的链接，一定要是实实在在的，不要刻意的去增加无谓的链接，更不要重复链接。

2． 泛相关链接：有可能我们所做的内容里面出现的关键词和您所有的文章不相关，但是和我们的网站是相关的，这里可以考虑给主页一个回链，切勿堆积链接。

3、 友情链接：适当的做内容页面的友情链接不仅能够增加内页的权重，更能侧向的增加我们网站的权重乃至权威，这里说的是最好是换完全相关的链接。
...
单页面展示内容不超过7屏。图片和文字是向用户介绍应用程序功能的很好的方式，但如果资源允许的话，视频将是一个更优方案。提供视频URL链接，保证单页面刷新速度小于1.5s。
...
========
    ...
    网站好的交互设计可以增加网站的友好度、可用性和易用性，从而使用户能够简单、快速和有效地完成网站赋予或用户自身所需求的服务、功能和目标。

1）入口说明
	入口不仅包含网站的导航，还有如按钮、选单、输入框、文字链接等。入口的摆放位置、命名规范需要清晰和准确，同时入口也需要为新进用户起到引导的作用。
	1、在每个志愿者招聘信息页面中，用户报名申请、用户组队和在线提问按钮需要醒目，在线提问按钮应跟随用户鼠标滑动而移动，当便用户发现问题第一时间可以进行询问。
	2、在个人信息页面，好友列表应出于醒目位置，好友聊天应在选择好友后自动弹出，方便用户直接点击。
	3、在聊天页面，用户组队按钮应醒目，方便用户组队。用户聊天页面中，用户的收藏夹应出于醒目位置，方便用户与好友分享信息。当好友分享信息时，应弹出收藏按钮，方便用户对好友分享的信息进行收藏。
	
2）界面重点说明
	1、志愿活动页面内容
	每个网站页面中都会有其中最重要的内容，对于针对高校的志愿者招聘网站，我们的重点应该为招聘方、志愿场所、时间等，需要将这些重点内容通过页面布局、构图、颜色，或者是“隐藏”其他次要内容来突显，让学生能够更快速、有效的找到仔细想了解的该志愿者活动的相关信息。
	2、页面颜色
	网站界面应用不同的颜色区分信息的种类，但应尽量少，以免颜色过多使页面混乱。本网站为志愿者招聘网站，且面向用户群是大学生，因此网站的主色调应为阳光、活力的暖色调为主。对于重要信息，应用明显、区分度较大的颜色区分。
	页面导航
	网站的页面导航应将活动信息、个人主页、搜索、帮助等使用频率较高的导航突出，其他导航可以借鉴百度页面中，用“更多”隐藏起来，使页面简洁，又方便用户寻找。
	内容填写页面
	在含有内容填写页面，提示文字应清晰易懂，用最直白的方式提示用户如何填写。填写的文字框应时时检错，一旦发现非法填写，应立即提示用户，而不是等用户确认以后再进行检错。在某些信息填写的文本框中，如果涉及用户个人信息，且用户个人信息中已有填写，应默认帮用户填写，并使其容易修改。填写框应设有填写辅助，例如邮箱后缀等，在如性别、生日等信息填写框，应为用户设置备选项，方便用户迅速填写。
	
3）内容说明
	网站页面内容要清晰，应尽量用清晰地图片logo代替文字，使用户非常明了的看出活动主办方、活动主题等信息，而不是需要大量阅读。
	网站的弹出提示内容应符合当代大学生的口吻，特别是失败提示，应利用遗憾的语气或图片进行提示而不能用很干白的文字。
